...
package com.klirimanjirou.android_login_tutorial.curl;
...
public class Curl {
	...
	public static String postInSeparateThread(String url, String header, String data) {
		Log.d("ACTOIN:","postInSeparateThread");
		PostAsyncTask async = new PostAsyncTask();
		Log.d("ACTOIN:","postInSeparateThread2");
		async.execute(url, header, data);
		Log.d("ACTOIN:","postInSeparateThread3");
		while (!async.isDone()){
			try {
				Thread.sleep(1);
				Log.d("ACTOIN:","postInSeparateThread4");

			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				Log.d("ACTOIN:","postInSeparateThread5");

				e.printStackTrace();
			}
		}
		Log.d("async.getResult():",async.getResult());
		return async.getResult();
